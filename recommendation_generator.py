...
def generate_recommendations(twitter_handle):
    ...
    if last_tweet_timestamp is None:
        last_tweet_timestamp = datetime.datetime.strptime("2017-01-01", "%Y-%m-%d")
    tweets = fetch_tweets_after_time(twitter_handle, last_tweet_timestamp)
    if not tweets:
        print "No tweets available yet"
        return
    tweet_text_list = [(tweet.text.encode("ascii", errors='ignore'), tweet.id) for tweet in tweets]
    classified_tweets = scripts.classifier.classify_tweets(tweet_text_list)
    print classified_tweets
    labeled_tweets = []
    for tweet in tweets:
        db_tweet = Tweet(tweet.text.encode("ascii", errors="ignore"), classified_tweets[tweet.id][0], tweet.created_at, user_id, classified_tweets[tweet.id][1].item())
